...
### ユーザーストーリー 4 - 3社比較レーダーチャート (優先度: P3)

**概要**: 
多次元KPI比較により、3社の財務健全性を視覚的に比較できるようにする。

**優先度の理由**: 
比較分析は有用だが、基本的な指標表示やトレンド分析に比べて優先度は低い。将来拡張として位置付ける。

**独立したテスト**: 
レーダーチャートを表示し、ROE、自己資本比率、DSCRの3軸で3社のデータが正しくプロットされることを確認できる。

**受入基準**:

1. **Given** レーダーチャートタブを選択した時、**When** チャートを描画する、**Then** 3社のROE、自己資本比率、DSCRがレーダーチャートで表示される
2. **Given** 会社を選択/解除した時、**When** チャートを更新する、**Then** 選択した会社のデータのみが表示される
...
- **FR-004**: システムは3社のKPIをレーダーチャートで比較表示しなければならない
